...
export default function PushNotifications({ navigation }) {
	...
	const renderIcon = (type, icon) => {
		if (type === 'order')
			return <Feather name={icon} size={24} color="#58A4B0" />;
		if (type === 'promo')
			return <MaterialIcons name={icon} size={24} color="#FF521B" />;
		if (type === 'update')
			return <MaterialIcons name={icon} size={24} color="#58A4B0" />;
					...
					renderItem={({ item }) => (
						...
						>
							...
							<View style={styles.textContainer}>
								...
								<Text style={[styles.notifMsg, { color: theme.text }]}>
									{item.message}
								</Text>
